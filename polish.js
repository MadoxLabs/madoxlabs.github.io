...
Game.init = function ()
{
  ...
  for (var x = 0; x < 400; ++x) 
    for (var y = 0; y < 400; ++y) 
    {
      this.height[x+400*y] = ((x+y) / 800) * 10;
      this.water[x+400*y] = 10 - ((x+y) / 800) * 10;
...
Game.update = function ()
{
  ...
  for (var x = 1; x < 400-1; ++x)
    for (var y = 1; y < 400-1; ++y)
    {
      ...
      if (maxWater == 0 || waterOut == 0) outflow.clear();
  ...
  for (var x = 1; x < 400-1; ++x)
    for (var y = 1; y < 400-1; ++y)
    {
      ...
      if (Game.height[x+400*y] > -10) oldwater *= evaporate;
...
Game.draw = function ()
{
  ...
  for (var x = 0; x < 400; ++x)
    for (var y = 0; y < 400; ++y)
    {
      ...
      waterA = Game.water[x+400*y] / 20.0;
      visualA = 1 - Game.visual[x+400*y] / 20.0;

      //                  | sand        |  water       |
      if (Game.visual[x+400*y] < 0)
      {
        Game.map.data[i++] = 237 * sandA  + 0 * visualA;
        Game.map.data[i++] = 201 * sandA  + 0 * visualA;
        Game.map.data[i++] = 175 * sandA  + 255 * visualA;
        Game.map.data[i++] = 255;
      }
      else
      {
