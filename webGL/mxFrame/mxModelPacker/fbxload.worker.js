...
{
  error("Got model data, size " + oEvent.data.length);
  error("Starting");
  process(oEvent.data);
}
...
function bake(name, mesh)
{
  ...
  error("Processing "+name+". Creating index for " + vertexstring.length + " verts. Result is " + uniq.length + " unique verts");
...
function process(data)
{
  ...
  for (; i < lines.length; ++i)
  {
    ...
    else if (lines[i].indexOf("Model: ") != -1)
    {
      ...
      error("Found model: " + curmodel.name);
    ...
    else if (lines[i].indexOf("Material: ") != -1)
    {
      ...
      error("Found material: " + curmaterial.name);
    ...
    if (state == States.ParseConnections)
    {
      if (lines[i].indexOf("C: ") != -1)
      {
        ...
        if (obj1.type === "mesh" && obj2.type === "model") { error("Model " + obj2.name + " has geometry"); obj2.mesh = obj1; }
